...
def AnalyzeResume():
    ...
    data = Ctrl_v1.AnalyzeResume(request.form, request.files)
    return Response(json.dumps(data), status=data['ApiHttpResponse'], mimetype='application/json')

@app.route("/api/v1/Analyze/<int:analysis_id>", methods=["GET"])
def GetAnalysis(analysis_id):
    
    data = Ctrl_v1.GetAnalysis(analysis_id)
    return Response(json.dumps(data), status=data['ApiHttpResponse'], mimetype='application/json')

####################################
# Initiate web server
####################################
app.run(host = '0.0.0.0',port = 8001,debug = settings.FLASK_DEBUG)